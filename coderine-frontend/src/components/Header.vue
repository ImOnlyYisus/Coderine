    ...
    <header class="generic__header">
        <div class="header__container">
            <div class="header__nav-links--left">
                <RouterLink to="/"
                    ><strong>01.</strong>{{ $t('header-component.links.home') }}</RouterLink
                >
                <RouterLink to="/playground"
                    ><strong>02.</strong>{{ $t('header-component.links.playground') }}</RouterLink
                >
                <RouterLink to="/components"
                    ><strong>03.</strong>{{ $t('header-component.links.components') }}</RouterLink
                >
            </div>
            ...
            <div class="header__nav-links-right">
                <RouterLink
                    to="/register"
                    v-if="!useAuthStore().isAutheticated"
                    class="generic__button--primary"
                    >{{ $t('header-component.links.register') }}
                </RouterLink>
                <RouterLink
                    to="/login"
                    v-if="!useAuthStore().isAutheticated"
                    class="generic__button--secondary"
                ...
                <div class="header__language-selector">
                    <div class="header__language-selector--button">
                        <input
                            type="checkbox"
                            class="header__language-selector--checkbox"
...
const onLogout = () => {
    confirmCancel(t('header-component.logout.title'), t('header-component.logout.message')).then(
        (result) => {
